...
public class DoScaleAction: DoTweenAction
{
    public override Tween GenerateDotween(Component component)
    {
        ...
        switch (data.isUsingFrom)
        {
            case true:
                if (rt != null)
                    tween = rt.DOScale(data.endValue, data.duration).From(data.fromValue).SetEase(data.ease);
                else if (transform != null)
                    tween = transform.DOScale(data.endValue, data.duration).From(data.fromValue).SetEase(data.ease);
                break;

            case false:
                if (rt != null)
                    tween = rt.DOScale(data.endValue, data.duration).SetEase(data.ease);
                else if (transform != null)
                    tween = transform.DOScale(data.endValue, data.duration).SetEase(data.ease);
                break;
        }
    ...
    public override void OnEditorGUI(ref Rect rect)
    {
        ...
        EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), "Scale");
